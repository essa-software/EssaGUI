...
private:
    virtual void draw(Gfx::Painter& painter) const override {
        painter.draw(Gfx::Drawing::Rectangle { local_rect().cast<float>(), Gfx::Drawing::Fill::solid(Util::Color { 0x00ff0040 }) });
        ...
        text.align(GUI::Align::Center, local_rect().cast<float>());
        text.draw(painter);
    }
...
int main() {
    GUI::Application app;
    auto& window = app.create_host_window(
        { 700, 400 }, "TEST",
