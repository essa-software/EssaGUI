...
#include <EssaGUI/gui/DraggableView2D.hpp>
...
private:
    virtual void draw(Gfx::Painter& painter) const override {
        ...
        painter.draw_rectangle({ -100, -100, 20, 20 }, rect);

        rect.fill_color = Util::Colors::Blue;
        painter.draw_rectangle({ 90, 90, 20, 20 }, rect);

        m_stats_label->set_content(Util::UString { fmt::format("Offset={} Zoom={}", fmt::streamed(offset()), zoom()) });
    }
