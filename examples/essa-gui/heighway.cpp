...
private:
    ...
    virtual void draw(Gfx::Painter& painter) const override {
        ...
        Gfx::Text text {
            Util::UString { std::to_string(m_point_count) + "; "
                + std::to_string(
                    static_cast<int>(GUI::Application::the().tps()))
                + " FPS; "
                + std::to_string(std::floor(static_cast<double>(m_point_count)
                    / (m_clock.elapsed() / 1.0s)))
                + " pps" },
    ...
    void append_point() {
        ...
        if (equation) {
            ...
            m_current_position.x()
                = 0.76 * old_position.x() - 0.4 * old_position.y();
            m_current_position.y()
                = 0.4 * old_position.x() + 0.76 * old_position.y();
        }

        m_painter.builder().set_projection(
            llgl::Projection { {}, { 0, 0, 500, 500 } });
        Gfx::RectangleDrawOptions point;
        point.fill_color = Util::Colors::White;
        m_painter.deprecated_draw_rectangle(
            { Util::Cs::Point2f::from_deprecated_vector(m_current_position),
                { 0.001, 0.001 } },
            point);
    }
...
int main() {
    GUI::Application app;
    auto& wnd = app.create_host_window(
        { 750, 750 }, "Heighway's Dragon (Gfx::Painter benchmark)");
