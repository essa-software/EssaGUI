...
template<typename T>
std::pair<Util::UString, typename StateTextButton<T>::State>
StateTextButton<T>::get_element_from_index(unsigned index) const {
    unsigned i = 0;
...
template<typename T>
void StateTextButton<T>::add_state(Util::UString content, T state,
    Util::Color bg_color, Util::Color fg_color, Util::Color text_color) {
    m_states.push_back({ content, { bg_color, fg_color, text_color, state } });
}

template<typename T> void StateTextButton<T>::set_index(unsigned int index) {
...
template<typename T>
Widget::EventHandlerResult StateTextButton<T>::on_mouse_button_press(
    Event::MouseButtonPress const&) {
    m_index++;
...
void StateTextButton<T>::draw(Gfx::Painter& painter) const {
    ...
    painter.deprecated_draw_rectangle(local_rect().template cast<float>(), bg);

    Gfx::Text text { get_element_from_index(m_index).first,
        Application::the().font() };
    text.set_font_size(theme().label_font_size);
    text.set_fill_color(get_element_from_index(m_index).second.text_color);
    text.align(Align::Center, local_rect().template cast<float>());
    text.draw(painter);
}
