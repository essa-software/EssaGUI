...
Util::UString Progressbar::get_content_with_labelling() const {
    ...
    case Labelling::Percentage:
        return m_content + " ("
            + Util::to_ustring((int)(((float)m_value / (float)m_max) * 100))
            + "%)";
    case Labelling::Value:
        return m_content + " (" + Util::to_ustring(m_value) + " / "
            + Util::to_ustring(m_max) + ")";
    }
    return m_content;
}
...
void Progressbar::draw(Gfx::Painter& painter) const {
    ...
    completed_rect.width = std::min<float>(
        (float)m_value / (float)m_max * raw_size().x(), raw_size().x() - 4);
    ...
    Gfx::Text text_opt { get_content_with_labelling(),
...
EML::EMLErrorOr<void> Progressbar::load_from_eml_object(
    EML::Object const& object, EML::Loader& loader) {
    TRY(Widget::load_from_eml_object(object, loader));
    m_content = TRY(object.get_property("content", "").to_string());
    m_labelling = TRY(
        object.get_enum("labelling", labelling_from_string, Labelling::None));
    m_min
        = TRY(object.get_property("min", EML::Value(static_cast<double>(m_min)))
                  .to_double());
    m_max
        = TRY(object.get_property("max", EML::Value(static_cast<double>(m_max)))
                  .to_double());
    m_step = TRY(
        object.get_property("step", EML::Value(static_cast<double>(m_step)))
            .to_double());
    m_progressbar_color
        = TRY(object.get_property("color", EML::Value(m_progressbar_color))
                  .to_color());
    m_value = TRY(
        object.get_property("value", EML::Value(static_cast<double>(m_value)))
            .to_double());
    return {};
}
