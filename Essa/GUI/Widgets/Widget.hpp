...
protected:                                      \
    Type m_##camel_case = def_val;              \
                                                \
public:                                         \
    Type camel_case() const {                   \
        return m_##camel_case;                  \
    }                                           \
    void set_##camel_case(Type const& value) {  \
        m_##camel_case = value;                 \
    }
...
public:                                        \
    ...
    void set_##camel_case(bool const& value) { \
        m_##camel_case = value;                \
    }
...
public:
    ...
    Util::Recti parent_relative_rect() const {
        return { parent_relative_position(), raw_size() };
    }
    // Returns bounding box with position = (0, 0)
    Util::Recti local_rect() const {
        return { {}, m_raw_size };
    }

    [[deprecated("Use absolute_position() instead")]] Util::Point2i raw_position() const {
    ...
    LengthVector input_position() const {
        return m_input_position;
    }
    LengthVector input_size() const {
        return m_input_size;
    }
    ...
    virtual Util::Size2i total_size() const {
        return raw_size();
    }
    ...
    WindowRoot& window_root() const {
        return *m_window_root;
    }
    /*restricted(MDI::Overlay) part-of-hack*/ WindowRoot* maybe_window_root() const {
        return m_window_root;
    }
    ...
    /*restricted(Custom Containers)*/ void set_window_root(WindowRoot& wtr) {
        m_window_root = &wtr;
    }
    /*restricted(Custom Containers)*/ void set_parent(Widget& parent);

    void set_double_click_enabled(bool dblclk) {
        m_double_click_enabled = dblclk;
    }

    virtual llgl::Cursor const& cursor([[maybe_unused]] Util::Point2i local_position) const {
...
protected:
    ...
    virtual EventHandlerResult on_key_press(Event::KeyPress const&) {
        return EventHandlerResult::NotAccepted;
    }
    virtual EventHandlerResult on_key_release(Event::KeyRelease const&) {
        return EventHandlerResult::NotAccepted;
    }
    virtual EventHandlerResult on_mouse_enter(Event::MouseEnter const&) {
        return EventHandlerResult::NotAccepted;
    }
    virtual EventHandlerResult on_mouse_leave(Event::MouseLeave const&) {
        return EventHandlerResult::NotAccepted;
    }
    virtual EventHandlerResult on_mouse_move(Event::MouseMove const&) {
        return EventHandlerResult::NotAccepted;
    }
    virtual EventHandlerResult on_mouse_button_press(Event::MouseButtonPress const&) {
        return EventHandlerResult::NotAccepted;
    }
    virtual EventHandlerResult on_mouse_button_release(Event::MouseButtonRelease const&) {
        return EventHandlerResult::NotAccepted;
    }
    virtual EventHandlerResult on_mouse_double_click(Event::MouseDoubleClick const&) {
        return EventHandlerResult::NotAccepted;
    }
    virtual EventHandlerResult on_mouse_scroll(Event::MouseScroll const&) {
        return EventHandlerResult::NotAccepted;
    }
    virtual EventHandlerResult on_text_input(Event::TextInput const&) {
        return EventHandlerResult::NotAccepted;
    }
    ...
    virtual bool should_display_tooltip(Util::Point2i const&) const {
        return !m_tooltip_text.is_empty();
    }

    // Called when tooltip is first shown up. Position is widget-relative.
    virtual Util::UString create_tooltip(Util::Point2i const&) const {
...
private:
    ...
    virtual LengthVector initial_size() const {
        return LengthVector {};
    }
