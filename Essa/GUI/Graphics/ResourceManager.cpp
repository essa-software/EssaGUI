...
ResourceManager::ResourceManager()
    : m_config { (find_resource_roots(),
        Util::ConfigFile::open_ini(
            require_lookup_resource({ ResourceId::asset("Config.ini"), "" }))
            .release_value()) } { }

std::optional<std::string> ResourceManager::lookup_resource(
    ResourceIdAndBase const& id_and_base) const {
    ...
    for (auto const& root : m_resource_roots) {
        auto filesystem_path = std::filesystem::path { root } / id_and_base.base
            / id_and_base.id.path();
...
std::string ResourceManager::require_lookup_resource(
    ResourceIdAndBase const& id_and_base) const {
    auto fs_path = lookup_resource(id_and_base);
    if (!fs_path) {
        std::cout << "ResourceManager: Failed to lookup required resource '"
                  << id_and_base << "'" << std::endl;
        exit(1);
    }
    return *fs_path;
}

std::optional<Texture> ResourceTraits<Texture>::load_from_file(
...
std::optional<Font> ResourceTraits<Font>::load_from_file(
    std::string const& path) {
...
llgl::TTFFont& ResourceManager::bold_font() const {
    return require<Font>(
        m_config.get("DefaultBoldFont").value_or("bold-font.ttf"));
}

llgl::TTFFont& ResourceManager::fixed_width_font() const {
    return require<Font>(
        m_config.get("DefaultFixedWidthFont").value_or("fixed-width-font.ttf"));
}
...
void ResourceManager::find_resource_roots() {
    auto add_resource_root_if_is_essa_resource_root
        = [this](std::filesystem::path const& path) {
              if (std::filesystem::is_directory(path)
                  && std::filesystem::is_regular_file(
                      path / ".essaguiresources")) {
                  std::cout << "ResourceManager: Adding resource root: " << path
                            << std::endl;
    ...
    if (Essa::BuildConfig::is_production) {
        ...
        //      directory
        {
            ...
            while (true) {
                add_resource_root_if_is_essa_resource_root(
                    base_path / "assets" / Essa::BuildConfig::target_name);
                if (base_path == "/")
                    break;
        ...
        if (Essa::BuildConfig::builtin_asset_root) {
            add_resource_root_if_is_essa_resource_root(
                *Essa::BuildConfig::builtin_asset_root);
        }
    }
