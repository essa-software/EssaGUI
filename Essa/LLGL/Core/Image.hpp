...
public:
    ...
    std::span<Util::Color const> pixels() const {
        return { m_pixels, pixel_count() };
    }
    std::span<Util::Color> pixels() {
        return { m_pixels, pixel_count() };
    }
    ...
    size_t pixel_count() const {
        return static_cast<size_t>(m_size.x()) * m_size.y();
    }
...
private:
    ...
    ESSA_ALWAYS_INLINE size_t pixel_index(Util::Point2u pos) const {
