    ...
    template<class Callback> void generate(SphereSettings<Callback> const& settings) {
        ...
        for (unsigned stack = 0; stack < settings.stacks; stack++) {
            for (unsigned sector = 0; sector < settings.sectors; sector++) {
                ...
                auto point_position
                    = Util::Vector3f::create_spheric(Util::Angle::radians(stack_angle), Util::Angle::radians(sector_angle), 1);
                Model::Vertex vertex { point_position, Util::Colors::White, {}, point_position };
