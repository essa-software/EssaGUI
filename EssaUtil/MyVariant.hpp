...
template<size_t val> struct static_max<val> {
...
template<typename... Types> class MyVariant {
    ...
    size_t invalid_type() { return typeid(void).hash_code(); }
...
public:
    ...
    template<typename T> MyVariant(T const& val) { set<T>(val); }
    ...
    template<typename T> bool is_type_of() const { return type_id == typeid(T).hash_code(); }
