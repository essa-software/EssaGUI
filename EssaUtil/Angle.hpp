...
public:
    ...
    constexpr static Angle degrees(double d) {
        return Angle { deg_to_rad(d) };
    }
    constexpr static Angle radians(double r) {
        return Angle { r };
    }

    constexpr double deg() const {
        return rad_to_deg(m_value_in_radians);
    }
    constexpr double rad() const {
        return m_value_in_radians;
    }
    ...
    constexpr Angle operator*(double n) const {
    ...
    constexpr Angle& operator*=(double n) {
        return *this = *this * n;
    }

    constexpr std::partial_ordering operator<=>(Angle const& other) const {
        return m_value_in_radians <=> other.m_value_in_radians;
    }
...
template<>
class fmt::formatter<Util::Angle> : public fmt::formatter<std::string_view> {
public:
    template<typename FormatContext>
    constexpr auto format(Util::Angle const& p, FormatContext& ctx) const {
