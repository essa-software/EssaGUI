...
public:
    ...
    OsErrorOr<void> write_little_endian(FP value) {
        return write_little_endian(std::bit_cast<uint32_t>(value));
    }

    template<std::floating_point FP>
        requires(sizeof(FP) == 8)
    OsErrorOr<void> write_little_endian(FP value) {
        return write_little_endian(std::bit_cast<uint64_t>(value));
    }
    ...
    template<std::floating_point FP>
        requires(sizeof(FP) == 4)
    OsErrorOr<void> write_big_endian(FP value) {
        return write_big_endian(std::bit_cast<uint32_t>(value));
    }

    template<std::floating_point FP>
        requires(sizeof(FP) == 8)
    OsErrorOr<void> write_big_endian(FP value) {
        return write_big_endian(std::bit_cast<uint64_t>(value));
    }
