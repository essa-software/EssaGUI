...
template<size_t C, class T>
constexpr Point<C, T> Point<C, T>::create_spheric(GeoCoords const& coords, double radius)
    requires(C == 3)
{
    return coords.to_cartesian<T>(radius);
}
...
template<size_t C, class T>
constexpr Vector<C, T> Vector<C, T>::create_spheric(GeoCoords const& coords, double radius)
    requires(C == 3)
{
    return coords.to_cartesian<T>(radius);
}
