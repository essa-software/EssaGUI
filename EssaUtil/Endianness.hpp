...
template<>
constexpr uint8_t swap_endianness<uint8_t>(uint8_t value) {
    return value;
}
template<>
constexpr uint16_t swap_endianness<uint16_t>(uint16_t value) {
    return __builtin_bswap16(value);
}
template<>
constexpr uint32_t swap_endianness<uint32_t>(uint32_t value) {
    return __builtin_bswap32(value);
}
template<>
constexpr uint64_t swap_endianness<uint64_t>(uint64_t value) {
